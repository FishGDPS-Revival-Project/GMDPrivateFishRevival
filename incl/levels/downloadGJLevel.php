...
//adding the download
		...
		$uploadDate = date("F jS, Y", $result["uploadDate"]);
		$updateDate = date("d-m-Y G-i", $result["updateDate"]);
		//password xor
		$pass = $result["password"];
        
        $rawdesc = $result["levelDesc"];
        if (empty($rawdesc)) {
            $rawdesc = base64_encode("(No description provided)");
        }
        
		$desc = (base64_decode($rawdesc)).' '.'<cg>('.$uploadDate.')</c>';

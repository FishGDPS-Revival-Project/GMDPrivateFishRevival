...
if($type == "top" OR $type == "creators" OR $type == "relative"){
	if($type == "top"){
		$query = $db->prepare("SELECT * FROM users WHERE isLeaderboardBanned = '0' AND gameVersion $sign AND stars > 0 ORDER BY stars DESC LIMIT 100"); // PATCH: used top 50 instead of top 100 for 1.5
        $query->execute();
	}
	if($type == "creators"){
    ...
    if($type == "relative"){
		$query = $db->prepare("SELECT * FROM users WHERE isLeaderboardBanned = '0' AND gameVersion $sign AND demons > 0 ORDER BY demons DESC LIMIT 100"); // PATCH: Demon leaderboard instead of global
        $query->execute();
	}
	$result = $query->fetchAll();
	if($type == "relative"){
		...
		$f = "SELECT rank, stars FROM (
							SELECT @rownum := @rownum + 1 AS rank, stars, extID, isLeaderboardBanned
